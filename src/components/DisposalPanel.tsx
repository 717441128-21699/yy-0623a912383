...
export default function DisposalPanel({ onDataChange }: Props) {
  ...
  const loadData = async () => {
    ...
    setAbnormalBatches(all.filter(b => b.status === '待处置' || b.status === '禁止使用'));
    setDisposals(disp);
    onDataChange?.();
  };
                        ...
                        return (
                          <tr key={b.id}>
                            ...
                            <td>
                              ...
                              {!hasDisp && <span className="sub-tag tag-miss">未处置</span>}
                              {hasDisp && <span className="sub-tag tag-warn">处置中</span>}
                            </td>
                            <td>
